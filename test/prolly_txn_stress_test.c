/* Cursor merge of TREE+MUT, nested SAVEPOINT × mutmap × DDL × VC.
** Built with DOLTLITE_PROLLY_CHECK=1 so mutmap/tree invariants fire. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
static void test_constraint_fail_then_scan(void){
  ...
  /* Unique violation rolls back only that statement. */
  check("cf_dup_fails",
...
static void test_nested_ddl_catalog(void){
  ...
  /* Prepared stmt on u must not return a false row after ROLLBACK drops u. */
  check("ddl_prep_u",
...
static void test_vc_commit_mid_savepoint(void){
  ...
  /* dolt_commit seals/invalidates the savepoint. */
...
static void test_constraint_in_nested_savepoint(void){
  ...
  /* Statement rolled back; sp2 still open. */
  check("cf_cnt",
    queryInt(db, "SELECT count(*) FROM t", &cnt)==SQLITE_OK && cnt==3);
...
static void test_deep_release_then_outer_rollback(void){
  ...
  /* ROLLBACK TO sp1 undoes everything from sp1 onward even after RELEASE. */
