...
for direction in forward reverse; do
  for shared in one both; do
    for edits in rows shared; do
      tag="shared_column_positions_${direction}_${shared}_${edits}"
      DB="$TMPROOT/$tag.db"
      ours=main
      theirs=feat
      ...
      column=n
      if [ "$edits" = shared ]; then column=x; fi
      extra=""
