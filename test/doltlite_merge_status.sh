...
run_test "fast_forward_not_merging" "SELECT dolt_merge('ff');
$MS" \
...
echo "BEGIN; SELECT dolt_merge('src');
COMMIT;" | $DOLTLITE "$DB10" > /dev/null 2>&1
...
echo "BEGIN; SELECT dolt_merge('$MID_HASH');
COMMIT;" | $DOLTLITE "$DB11" > /dev/null 2>&1
...
echo "BEGIN; SELECT dolt_merge('src');
COMMIT;" | $DOLTLITE "$DB12" > /dev/null 2>&1
...
echo "BEGIN; SELECT dolt_merge('src');
COMMIT;" | $DOLTLITE "$DB13" > /dev/null 2>&1
...
echo "BEGIN; SELECT dolt_merge('src');
COMMIT;" | $DOLTLITE "$DB14" > /dev/null 2>&1
...
echo "BEGIN; SELECT dolt_merge('src');
COMMIT;" | $DOLTLITE "$DB15" > /dev/null 2>&1
