...
echo "phase 2"
X="NOT A NUMBER"
[ "1" = "$((X*2))" ] && echo ok || echo bad
echo "Results: 9 passed, 0 failed"
echo "__SUITE_COMPLETE__"
SH

cat > "$tmp/tally_then_truncation.sh" <<'SH'
set -u
...
expect_rc "tally_then_truncation_is_a_failure" 1 "$tmp/tally_then_truncation.sh"
...
if bash "$tmp/dies_midway.sh" 2>/dev/null | grep -qx '__SUITE_COMPLETE__'; then
  fail=$((fail+1))
  echo "  FAIL: unguarded_death_skips_completion (fixture no longer reproduces the bug)"
else
  pass=$((pass+1))
fi
