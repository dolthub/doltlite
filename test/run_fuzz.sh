...
if [ $# -lt 1 ]; then
  ...
  echo "  target: replaywal | prolly_node | deserialize_refs | read_index" >&2
  exit 2
fi
...
case "$target" in
  ...
  read_index)
    ...
    ;;
  *)
    echo "ERROR: unknown target '$target' (expected: replaywal | prolly_node | deserialize_refs | read_index)" >&2
    exit 2
    ;;
esac
