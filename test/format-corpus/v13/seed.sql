...
CREATE TABLE numeric_keys(k INTEGER, suffix TEXT, PRIMARY KEY(k,suffix DESC)) WITHOUT ROWID;
INSERT INTO numeric_keys VALUES
  (-9223372036854775808,'x'),(-9223372036854775807,'x'),
  (9007199254740991,'x'),(9007199254740992,'x'),(9007199254740993,'x'),
  (9007199254740994,'x'),(9007199254740995,'x'),(9223372036854775807,'x');
CREATE TABLE numeric_indexed(id INTEGER PRIMARY KEY, k INTEGER, suffix TEXT);
CREATE UNIQUE INDEX numeric_idx ON numeric_indexed(k,suffix DESC);
INSERT INTO numeric_indexed(k,suffix) SELECT k,suffix FROM numeric_keys;

SELECT dolt_commit('-A', '-m', 'v13 comprehensive base',
                   '--author', 'Format Writer <format@example.com>');
SELECT dolt_tag('v13-base', '-m', 'annotated format baseline',
...
SELECT dolt_commit('-A', '-m', 'v13 feature',
...
SELECT dolt_commit('-A', '-m', 'v13 main',
                   '--author', 'Main Writer <main@example.com>');
SELECT dolt_merge('feature');
SELECT dolt_tag('v13-merge', '-m', 'annotated merge result',
...
PRAGMA foreign_keys=ON;
SELECT dolt_commit('-A', '-m', 'v13 violating head',
