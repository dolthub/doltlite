#!/bin/bash
# Writable file in a read-only dir: same SQLITE_READONLY as stock when the graph-lock sidecar cannot be created.
...
# CLI must fail; extended READONLY_DIRECTORY is asserted in the C repro.
