...
int main(){
  ...
  /* Verify commit from another connection */
  check("commit_log_db2",
    strcmp(exec1(db2, "SELECT message FROM dolt_log LIMIT 1"),
  ...
  /* Verify from another connection too */
  check("log_count_db3", strcmp(exec1(db3, "SELECT count(*) FROM dolt_log"), "2")==0);
  ...
  check("final_log_count", strcmp(exec1(db2, "SELECT count(*) FROM dolt_log"), "3")==0);
  check("final_log_msg",
    strcmp(exec1(db2, "SELECT message FROM dolt_log LIMIT 1"),
