...
** clear the helper macros earlier sections defined so vec1's own
** definitions don't warn. */
#ifdef SQLITE_AMALGAMATION
# define i8 vec1_i8
# undef MAX
# undef MIN
# undef SWAP
# undef UNUSED_PARAMETER2
#endif
...
static int vec1ShadowName(const char *zName){
  ...
  unsigned int i;
  for(i=0; i<sizeof(azShadow)/sizeof(azShadow[0]); i++){
    if( sqlite3_stricmp(zName, azShadow[i])==0 ) return 1;
  }
  return 0;
}
...
/* doltlite: per-connection registration entry for the built-in
** extension list in main.c, mirroring how sqlite_dbpage is registered
** without a process-global auto-extension. */
#ifdef VEC1_STATIC
int sqlite3Vec1Init(sqlite3 *db){
  return initExtension(db, 0, 0);
}
#endif
#ifdef SQLITE_AMALGAMATION
# undef i8
