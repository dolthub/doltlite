...
- **Version-control surfaces** — `src/doltlite_*.c`, roughly one file per
  ...
  (catalog orchestration in `doltlite_merge`, pass1/pass2 in
  `doltlite_merge_pass1` / `doltlite_merge_pass2`, rows in
  `doltlite_merge_rows`, schema IR in `doltlite_merge_schema`, plus
